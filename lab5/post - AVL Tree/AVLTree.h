...
public:
  ...
  //AVLNode* findParent(const string& x) const;
...
private:
  ...
  int count;
   ...
   AVLNode* rotateLeft(AVLNode*& n);
    ...
    int calcHeight(AVLNode* t) const;

    // private helper for printTree to allow recursion over different nodes.
  void insert(AVLNode*&i, const string& x);
  string pathTo(AVLNode* p, const string& x) const;
  bool find(AVLNode* b, const string& x) const;
  //AVLNode* findParent(AVLNode* f, const string& x) const;
